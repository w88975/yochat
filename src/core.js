...
// 初始化URL
const LOGIN_URL = 'https://login.weixin.qq.com/jslogin?appid=wx782c26e4c19acffb&redirect_uri=https%3A%2F%2Fwx.qq.com%2Fcgi-bin%2Fmmwebwx-bin%2Fwebwxnewloginpage&fun=new&lang=en_US&_=' + new Date().getTime()
const QR_IMAGE_URL = 'https://login.weixin.qq.com/qrcode/'
...
var yochat = function () {
    this._this = this
    this.config = {}
    this.jar = request.jar() // jar 保存cookie用
...
yochat.prototype.fetch = async (url, options) => {
...
// 获取登录二维码的UUID
yochat.prototype.getQRCodeUUID = async function () {
    let QRLoginRes = await this.fetch(LOGIN_URL)
    var bodyJs = null
    var uuid = null
    QRLoginRes.body ? bodyJs = QRLoginRes.body : 0
    if (bodyJs.indexOf('200') > -1) {
        uuid = bodyJs.match(/(?:")[a-z,A-Z,0-9,/,+,_,-]*[=]{0,}(?:")/)[0]
    }
    return uuid.replace(/\"/g, '')
}

// 获取登录二维码地址
yochat.prototype.getQRCode = async function () {
    let UUID = await this.getQRCodeUUID()
    return {
        UUID,
        QRstring: `https://login.weixin.qq.com/l/${UUID}`,
        QRUri: QR_IMAGE_URL + UUID,
        // 在命令行生成二维码
        generateQRCodeForCli: () => {
            qrcode.generate(`https://login.weixin.qq.com/l/${UUID}`)
        },
        // 在浏览器中打开二维码
        generateQRCodeForBrowser: () => {
            open(QR_IMAGE_URL + UUID)
        }
    }
}

// 等待扫码操作
yochat.prototype.waitScan = async function (UUID) {
    var _this = this
    var RETRY = 10 // 重试次数 超过这个次数 则退出登录
    while (true) {
        let code = await _this.fetch(REDIRECT_URL.replace('$0', UUID), { jar: _this.jar })
        var window = {}
        eval(code.body)
        // 用户扫码动作 但未确认登录 获取头像
        if (window.code === 201 && window.userAvatar) {
            _this.avatar = window.userAvatar
        }
        else if (window.redirect_uri) {
            let redirect_data = await _this.fetch(window.redirect_uri, { json: true, followRedirect: false, jar: _this.jar })
            parseString(redirect_data.body, async (err, result) => {
                console.log(result)
                // { error: { ret: [ '1203' ], message: [ '' ] } } 登录次数过多 报错
                _this.config.uuid = UUID
                _this.config.skey = result.error.skey[0]
                _this.config.pass_ticket = result.error.pass_ticket[0]
                _this.config.wxsid = result.error.wxsid[0]
                _this.config.wxuin = result.error.wxuin[0]
                _this.config.cookie = redirect_data.headers['set-cookie']
                _this.config.isLogin = true
                let init_data = await (_this.fetch('https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxinit?r=' + new Date().getTime(), {
                    method: 'POST',
                    json: true,
                    ...
                    body: {
                        "BaseRequest":
                        {
                            "Uin": "xuin=" + _this.config.wxuin,
                            "Sid": _this.config.wxsid,
                            "Skey": "",
                            "DeviceID": _this.config.DeviceID ? _this.config.DeviceID : _this.config.DeviceID = "e" + new Date().getTime()
                        }
                    }
                }))
                _this.config.user = init_data.body.User // 获取用户信息
                _this.config.initData = init_data.body
            })
            break;
        }
    }
    ...
    // 获取用户基本信息
    let data = await (_this.fetch('https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxinit?r=' + new Date().getTime(), {
        method: 'POST',
        json: true,
        followRedirect: false,
        jar: _this.jar,
    ...
    _this.config.userInfo = data.body
    _this.config.SyncKey = data.body.SyncKey

    // 注册plugin
    var yo = {}
    for (var plg in this.plugin) {
        yo[plg] = this.plugin[plg](this)
    }
    yo.event = this.event
    return yo
}

yochat.prototype.login = async function (callback) {
    var _this = this
    var QR = await _this.getQRCode()
    QR.generateQRCodeForBrowser()
    let yo = await _this.waitScan(QR.UUID)
    
    callback && callback(yo)

}

yochat.prototype.use = function (middleware) {
    this.plugin[middleware.name] = middleware.handler
    console.log(`
加载插件: 
{
    name: ${middleware.name},
    author: ${middleware.author},
    description: ${middleware.des},
}`)
}

module.exports = yochat
